...
```sh
npm install @nx-community/graph-to-mermaid
```

## CLI Usage

Generate an Nx graph JSON file and pipe it through `graph-to-mermaid`:

```sh
# Generate the graph JSON and convert to Mermaid
nx graph --file=graph.json
graph-to-mermaid graph.json

# Or pipe from stdin
nx graph --file=- | graph-to-mermaid

# Choose a different graph direction (TD, LR, BT, RL)
graph-to-mermaid --direction=LR graph.json
```
...
- name: Generate Nx Graph as Mermaid
  run: |
    ...
    npx graph-to-mermaid graph.json >> $GITHUB_STEP_SUMMARY
...
```typescript
import { graphToMermaid } from '@nx-community/graph-to-mermaid';
